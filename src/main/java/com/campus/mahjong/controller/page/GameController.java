    ...
    private void renderTileCounter() {
        ...
        if (lanSession == null) {
            ...
            networkGame.players().forEach(player -> {
                ...
                player.exposedGroups().forEach(group -> group.stream().filter(tile -> !tile.code().equals("暗牌")).forEach(tile -> visible.add(TileType.fromDisplayName(tile.code()))));
            });
        }
        ...
        if (lanSession != null) {
            networkGame.players().stream().filter(player -> player.seat() == seat).findFirst()
                    .ifPresent(player -> player.exposedGroups().forEach(group -> {
                        if (group.stream().allMatch(tile -> tile.code().equals("暗牌"))) {
                            HBox backs = new HBox(2); backs.setAccessibleText("暗杠");
                            for (int i = 0; i < group.size(); i++) {
                                Label back = new Label("暗"); back.getStyleClass().add("tile-back"); back.setPrefSize(32,44); backs.getChildren().add(back);
                            }
                            pane.getChildren().add(new Group(backs)); return;
                        }
