...
| 房间生命周期 | `MahjongTypes.RoomStatus` | `WAITING`、`PLAYING`、`CLOSED` | 状态集合与 `RoomManager` 的实际转移保持一致 |
| 对局生命周期 | `MahjongTypes.GameStatus` | `PLAYING`、`FINISHED` | 快照由 `InMemoryGameSessionService` 根据 `MahjongMatch.finished()` 生成 |
| 单局牌局 | `RoundPhase` | `EXCHANGING_TILES`、`CHOOSING_MISSING_SUIT`、`WAITING_FOR_DISCARD`、`WAITING_FOR_CLAIMS`、`FINISHED` | 摸牌在服务端内部同步完成，随后直接进入等待出牌 |
...
```mermaid
stateDiagram-v2
    [*] --> ExchangingTiles: 四川模式发牌完成

    state "换三张阶段\nEXCHANGING_TILES" as ExchangingTiles
    ...
    ExchangingTiles --> ExchangingTiles: 玩家提交同花色三张 [仍有人未选择]
    ExchangingTiles --> ChoosingMissingSuit: 四人均确认或10秒超时 / 原子交换
    ...
    WaitingForDiscard --> WaitingForClaims: 15秒超时 / 自动出牌
    ...
    WaitingForClaims --> WaitingForDiscard: 全部过或10秒超时 / 下家摸牌
...
> `drawFor(Seat)` 会在一次服务端临界区内完成摸牌，并直接把阶段设置为 `WAITING_FOR_DISCARD`，因此不需要单独的等待摸牌状态。

### 4.1 四川麻将子状态转移表

| 编号 | 当前阶段 | 事件/方法 | 前置或守卫条件 | 主要处理与优先级 | 下一阶段 | 版本变化 |
| --- | --- | --- | --- | --- | --- | --- |
| R01 | 初始化 | `MahjongRound.start(settings, seed)` | 设置非空；四川规则可解析；牌墙足够发牌 | 洗牌、每家发13张、东家额外摸1张并启动换牌计时 | `EXCHANGING_TILES` | 初始 `revision=0` |
| R02 | `EXCHANGING_TILES` | `submitExchange(seat, tiles, revision)` | 版本匹配；玩家未确认；恰好选择同花色3张实际持有牌 | 保存玩家选择；四人完成后按随机方向原子交换 | 原状态或 `CHOOSING_MISSING_SUIT` | 完成交换时 `+1` |
| R03 | `EXCHANGING_TILES` | `expireExchange(now)` | 当前时间达到截止时间 | 为未提交玩家自动选择3张并完成交换 | `CHOOSING_MISSING_SUIT` | `+1` |
| R04 | `CHOOSING_MISSING_SUIT` | `chooseMissingSuit(seat, suit, revision)` | 版本匹配；玩家未定缺；花色是万/筒/条 | 保存该玩家定缺 | 原状态或 `WAITING_FOR_DISCARD` | 四人完成时 `+1` |
| R05 | `CHOOSING_MISSING_SUIT` | `expireMissingSuitSelection(now)` | 当前时间达到截止时间 | 为未选择玩家填入推荐花色 | `WAITING_FOR_DISCARD` | `+1` |
| R06 | `WAITING_FOR_DISCARD` | `discard(seat, tile, revision)` | 版本匹配；轮到该玩家；牌在其牌组中；存在缺门牌时必须先打缺门牌 | 移除弃牌；将摸牌并入已整理手牌；记录最后出牌；自动为无响应动作玩家填 `PASS` | `WAITING_FOR_CLAIMS`，或立即结算响应 | `+1`，结算响应可能继续增加 |
| R07 | `WAITING_FOR_DISCARD` | `expireDiscard(now)` | 已超过15秒出牌时限 | 按合法出牌集合自动选择并打出一张 | `WAITING_FOR_CLAIMS` | 与正常出牌相同 |
| R08 | `WAITING_FOR_CLAIMS` | `submitClaim(seat, PASS, revision)` | 非出牌者；尚未响应；`PASS` 在合法动作中 | 记录响应；三家都响应后进行仲裁 | 原状态或 `WAITING_FOR_DISCARD` | 仲裁完成后增加 |
| R09 | `WAITING_FOR_CLAIMS` | `expireClaims(now)` | 已超过10秒响应时限 | 保留已提交选择，其余玩家自动过并仲裁 | `WAITING_FOR_DISCARD` 或 `FINISHED` | 仲裁完成后增加 |
| R10 | `WAITING_FOR_CLAIMS` | `submitClaim(seat, HU, revision)` | 加入弃牌后满足胡牌；无定缺花色残留 | 支持多人点炮胡；胡优先于杠、碰 | `WAITING_FOR_DISCARD` 或 `FINISHED` | 胡牌后继续推进 |
| R11 | `WAITING_FOR_CLAIMS` | `submitClaim(seat, GANG/PENG, revision)` | 动作由 `legalActions` 提供 | 优先级为 `HU > GANG > PENG`；同类按出牌者之后的逆时针座次选取 | `WAITING_FOR_DISCARD` | `+1`；明杠补摸还会推进版本 |
| R12 | `WAITING_FOR_CLAIMS` | 所有玩家 `PASS` | 三名非出牌者都已响应或被自动过 | 当前玩家变为出牌者的下一座位并摸牌 | `WAITING_FOR_DISCARD` | 摸牌时 `+1` |
| R13 | `WAITING_FOR_DISCARD` | `declareSelfDraw(seat, revision)` | 轮到本人；`legalActions` 包含 `HU`；无定缺牌；满足规则引擎 | 计算牌型和番数，形成零和积分变化，将玩家加入已胡集合 | `WAITING_FOR_DISCARD` 或 `FINISHED` | 后续摸牌/结束时增加 |
| R14 | `WAITING_FOR_DISCARD` | `declareConcealedGang(...)` | 当前玩家持有4张相同非缺门牌 | 移除4张牌，登记暗杠，补摸 | `WAITING_FOR_DISCARD` | 补摸时 `+1` |
| R15 | `WAITING_FOR_DISCARD` | `declareSupplementalGang(...)` | 当前玩家已有该牌的碰牌组，且持有第4张 | 将碰升级为杠并补摸 | `WAITING_FOR_DISCARD` | 补摸时 `+1` |
| R16 | 任意需要摸牌的路径 | `drawFor(seat)` | 玩家没有尚未处理的摸牌 | 从牌墙取1张放入右侧摸牌位 | `WAITING_FOR_DISCARD` | `+1` |
| R17 | 任意需要摸牌的路径 | `drawFor(seat)` | 牌墙为空 | 汇总本轮已有胡牌分数并生成流局/结束原因 | `FINISHED` | `+1` |
| R18 | 胡牌结算 | `continueAfterWin(previous)` | 已成功记录胡牌 | 已胡人数小于3时跳过已胡玩家并继续；达到3人时结束 | `WAITING_FOR_DISCARD` 或 `FINISHED` | 由摸牌或结束操作增加 |
...
| 方法签名 | 参数/返回值 | 前置条件 | 后置条件 |
| --- | --- | --- | --- |
| `static MahjongRound start(FriendRoomSettings settings, long randomSeed)` | 输入规则和随机种子；返回新牌局 | 设置有效；地区规则已注册 | 构造牌墙并发牌；四川模式进入换三张阶段；相同种子可复现牌序 |
| `void submitExchange(Seat seat, List<TileType> tiles, long expectedRevision)` | 座位、同花色3张牌、版本；无返回值 | 换三张阶段；版本匹配；玩家未确认；牌确实在手中 | 保存选择；四家完成后按随机方向原子交换并进入定缺 |
| `boolean expireExchange(long nowMillis)` | 当前时间；返回是否发生超时推进 | 处于换三张阶段且达到截止时间 | 自动填写未提交玩家的选择，完成交换并进入定缺 |
...
| `boolean expireDiscard(long nowMillis)` / `boolean expireClaims(long nowMillis)` | 当前时间；返回是否发生推进 | 对应阶段已经超过时限 | 自动出牌，或保留已有响应并令其他玩家自动过 |
...
| `void synchronizeRound()` | 无参数、无返回值 | 整场对象有效 | 记录尚未入账的胡牌；单轮结束后开始下一轮或设置 `finished=true` |
| `boolean expireTimedActions(long nowMillis)` | 当前时间；返回是否发生超时推进 | 整场未结束 | 依次处理换三张、定缺、出牌和响应阶段的超时动作 |
...
| `RoundOutcome win(Seat winner, Seat supplier, boolean selfDraw, FriendRoomSettings settings, List<String> patterns, int fan)` | 胜者、点炮者、自摸标记、规则、牌型、番数 | `0 <= fan <= 30`；自摸时点炮者可为空；非自摸时点炮者有效 | 按 `10 × 房间倍率 × 封顶后的番型倍率` 生成四家零和分数变化 |
| `RoundOutcome win(..., Set<Seat> activeSeats)` | 在上一签名基础上增加未胡玩家集合 | 活跃集合有效 | 血战到底中已胡玩家不再承担后续自摸付款 |

计分不变量：所有座位的 `scoreChanges` 总和必须为0；房间封顶值仅限制 `2^番数` 对应的番型倍率；算术溢出由 `Math.multiplyExact` 显式暴露。
...
5. 公共状态和动作枚举仅保留当前实现实际使用的取值，避免文档和协议暴露不可达状态。
